...
4. Run the app: `swift run` or `open .build/debug/StylometrySanitizer`
...
7. For cross-app neutralization, enable Accessibility permission when prompted, then press Cmd+Option+Shift+L while focus is on the target text field.

> If you opened an older `.app` bundle before this fix, rebuild and run via `swift run` to use the latest version.
